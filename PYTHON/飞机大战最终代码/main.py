...
pygame.display.set_caption("飞机大战 -- 雷霆战机简易版")

background = pygame.image.load("images/QQ图片20200425221016.png").convert()
...
# 载入游戏音乐
pygame.mixer.music.load("sound/Pacific_rim.ogg")
...
def main():
    ...
    #生成护盾
    protection_cover = pygame.image.load("images/Protection_cover.png").convert_alpha()
    ...
    while running:
        ...
        if life_num>= 0 and not paused:
            ...
            if not(delay % 10):
                ...
                if is_double_bullet:
                    bullets = bullet2
                    bullets[bullet2_index].reset((me.rect.centerx-56, me.rect.centery))
                    bullets[bullet2_index+1].reset((me.rect.centerx+8, me.rect.centery))
                    bullet2_index = (bullet2_index + 2) % BULLET2_NUM
                else:
                    bullets = bullet1
                    bullets[bullet1_index].reset((me.rect.centerx-16, me.rect.centery))
                ...
                if each.active:
                    ...
                    if not(delay % 10) and each.rect.bottom>0:
                        for i in range(BULLET3_NUM//2):
                            bullet3.append(bullet.Bullet3((each.rect.centerx-56, each.rect.centery+each.rect.height-170)))
                            bullet3.append(bullet.Bullet3((each.rect.centerx+8, each.rect.centery+each.rect.height-170)))
                        bullet3[bullet3_index].reset((each.rect.centerx-56, each.rect.centery+each.rect.height-170))
                        bullet3[bullet3_index+1].reset((each.rect.centerx+8, each.rect.centery+each.rect.height-170))
            ...
            if me.active:
                if me.invincible:
                    screen.blit(protection_cover, (me.rect.left-5, me.rect.top))
                if switch_image:
